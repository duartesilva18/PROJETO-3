...
function handleScheduleFieldChange(redeId, field, value) {
  ...
  const id_projeto_num = Number(formField.id_projeto);
  const hasProjetoVal = hasProjeto && Number.isFinite(id_projeto_num) && id_projeto_num > 0;
    ...
    try {
      ...
      if (hasProjetoVal) {
        formData.append('id_projeto',  id_projeto_num);
      }
...
function handleDragLeave(event) {
  ...
  function definirTemProjeto(valor) {
    hasProjeto = valor;
    if (!valor) {
      ...
      <div class="col-md-6">
          ...
          <div class="d-flex gap-3 projeto-toggle">
            <label class="form-check-label">
              <input
                ...
                checked={hasProjeto}
                onchange={() => definirTemProjeto(true)}
              />
              Sim
            </label>
            <label class="form-check-label">
              <input
                ...
                class="form-check-input"
                checked={!hasProjeto}
                onchange={() => definirTemProjeto(false)}
              />
              Não
            </label>
          </div>
        ...
        {#if hasProjeto}
          <div class="form-group">
            ...
            <select class="form-control" required={hasProjeto} bind:value={formField.id_projeto}>
        ...
        <div class="selected-file">
          ...
          <div class="file-body">
            ...
            <div class="file-networks">
              {#if getSelectedSocialNetworksNames().length === 0}
                  ...
                  <div class="schedule-row">
                    ...
                    <div class="schedule-inputs">
                        <input
                          type="date"
                          class="form-control"
                          value={schedulingForm.entries[rede.id_rede_social]?.horario?.split('T')[0] ?? ''}
                          oninput={(event) => {
                            const date = event.currentTarget.value;
                            const time = (schedulingForm.entries[rede.id_rede_social]?.horario?.split('T')[1]) || '08:00';
                            handleScheduleFieldChange(rede.id_rede_social, 'horario', `${date}T${time}`);
                          }}
                        />
                        <select
                          class="form-control"
                          value={schedulingForm.entries[rede.id_rede_social]?.horario?.split('T')[1]?.substring(0, 5) ?? '08:00'}
                          onchange={(event) => {
                            const date = (schedulingForm.entries[rede.id_rede_social]?.horario?.split('T')[0]) || new Date().toISOString().split('T')[0];
