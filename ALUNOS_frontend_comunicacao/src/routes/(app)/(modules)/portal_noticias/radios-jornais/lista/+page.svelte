...
import RemoveModal from '../../rede-social/detalhe/[id]/modals/RemoveModal.svelte';
	...
	function isTipoMedia(noticia) {
		// API pode devolver tipo como número 1 ou string "1"; aceitar ambos para não excluir após edição
		const t = noticia?.tipo;
		return t === 1 || t === '1' || Number(t) === 1;
	}
	...
	// Envia emails de mídia (texto personalizado por rádio/jornal ou texto base)
	async function enviarEmailsParaMidia(noticia) {
		...
		// Carregar textos personalizados por rádio/jornal (midia_personalizada)
		let textosPorRadio = {};
		try {
			const res = await fetch(`/ep/portal_noticias/midia_personalizada/${noticia.id_noticia}`, { cache: 'no-store' });
			if (res.ok) {
				const items = await res.json();
				(items || []).forEach((item) => {
					if (item.id_radio_jornal && (item.texto_custom ?? '').trim() !== '') {
						textosPorRadio[String(item.id_radio_jornal)] = item.texto_custom.trim();
					}
				});
			}
		} catch (e) {
			console.warn('Não foi possível carregar textos personalizados para o email:', e);
		}
		...
		const pedidos = ids.map(async (id) => {
			...
			// Usar texto personalizado para este rádio/jornal se existir, senão texto base
			const textoParaEmail = textosPorRadio[id] ?? baseTexto;
			...
			try {
				await fetch('/ep/notificacao/enviarNotificacaoEmail', {
					method: 'POST',
					headers: { 'Content-Type': 'application/json' },
					body: JSON.stringify(corpo)
				});
			} catch (err) {
				console.error(`Erro na chamada para ${radio.nome}:`, err);
			}
		});
	...
	async function onDeleteRowMidia() {
		const noticias_fetch = await fetch('/ep/portal_noticias/dados', { cache: 'no-store' }).then((d) => d.json());
	...
	onMount(async () => {
		...
		const [noticias_fetch, radio_jornal_fetch] = await Promise.all([
			fetch('/ep/portal_noticias/dados', { cache: 'no-store' }).then((d) => d.json()),
	...
	function refreshRadioTable() {
		...
		jQuery(radioTableEl)
			...
			.on('click', 'tbody tr', function (event) {
				// Evita interferir com cliques em botões ou links
				const target = jQuery(event.target);
				if (target.closest('button, a').length > 0) return;
		...
		radioTable.page('first').draw(false);
	}
	...
	<div class="row filter">
		<form onsubmit={preventDefault(onHandleSubmit)} class="w-100">
			<div class="row filter-row align-items-start g-3">
				<!-- ASSUNTO -->
				<div class="col-md-4">
					<label for="tituloInput" class="filter-label">
						{$t('divNoticias.Assunto')}
					</label>
					<input
						id="tituloInput"
						type="text"
						class="form-control"
						placeholder={$t('divNoticias.pesquisarPlaceholder') || 'Pesquisar...'}
						bind:value={formFilter.titulo}
					/>
				</div>
